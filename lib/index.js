...
export function apply(ctx) {
  ...
  const submitContinuation = (sessionId, text) => {
    ...
    try {
      ...
      const agent = registry && typeof registry.get === 'function' ? registry.get(sessionId) : undefined
  ...
  const executeSignal = (signal) => {
    try {
      if (signal.level === 'L1') {
        ...
        // 技术载荷: 续跑轮次的 persona 切 Minimal + 工具收缩(assemble 消费, 必须先于 followup 设置)
        pendingResets.set(signal.sessionId, (signal.payload && signal.payload.reset) || { systemPrompt: '', tools: [] })
        submitContinuation(signal.sessionId, 'anchored-monitor: react-band trajectory detected (' + (signal.reason || '') + '). An L2 reset is applied — this request runs under the minimal 46-character persona with the bash and str_replace_editor bootstrap pair. Continue the task from where you left off, in a plan-first collective style (we will …). 锚定监控已执行 L2 强制重置: 本轮 persona 切回 Minimal 46 字符句 + 双工具, 请以规划式风格(we will / we need)从刚才的位置继续任务。')
        ackSignal(signal, 'executed')
      } else if (signal.level === 'L3') {
        // L3: 同样重置条件续跑, 并附重启建议(不把对话断在原地)
        pendingResets.set(signal.sessionId, (signal.payload && signal.payload.reset) || { systemPrompt: '', tools: [] })
        submitContinuation(signal.sessionId, 'anchored-monitor: L2 retries exhausted (' + (signal.reason || '') + '). A full session restart is recommended for the cleanest state; meanwhile the reset condition is applied — continue the task in a plan-first style. 锚定监控提示: L2 重试已耗尽, 建议全新会话; 当前已应用重置条件, 请以规划式风格继续任务。')
        ackSignal(signal, 'executed')
      }
    } catch (err) {
