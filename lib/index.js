...
export function apply(ctx) {
  ...
  ctx.on('llm/stream', (options, next) => {
    ...
    // FIX(2026-08-18): 监听器必须是非 async 函数 + 直接迭代 next()。
    // llm/stream 是 waterfall: 链上前面的监听器(dsh-draw-gacha 等)会
    // for await (const chunk of next()) 直接迭代本监听器的返回值;
    // async 函数会把 async generator 包成 Promise, 导致
